  ...
  if (fiche) {
    ...
    if (d) {
      ...
      return {
        ...
        desc: `${name}, sénateur·rice ${circo}. ${d.presence}% de présence aux scrutins publics${d.participation != null ? `, ${d.participation}% de vote exprimé` : ""}, votes et loyauté au groupe — 100 % sourcé (Sénat).`,
        canonical: `${BASE}/depute/${d.slug}`,
        og: `${BASE}/og/${d.slug}.png`,
      };
    }
  }
  const pages = {
    "/deputes": ["Tous les sénateurs — FicheSénateur.fr", "Cherchez et comparez les 348 sénateurs du Sénat : présence, votes, groupe et circonscription."],
    ...
    "/groupes": ["Les groupes politiques — FicheSénateur.fr", "Présence moyenne et effectifs de chaque groupe politique du Sénat."],
...
// FAQPage JSON-LD on the Méthode route so crawlers & AI lift the answers (esp.
// "présence non publiée nominativement" + vote-par-délégation). FR canonical.
function faqJsonLd(pathname) {
  const p = pathname.replace(/\/+$/, "") || "/";
  if (p !== "/methode") return "";
  ...
  const ld = {
    ...
    "@type": "FAQPage",
    mainEntity: faq.fr.map((x) => ({
      "@type": "Question",
      name: x.q,
      acceptedAnswer: { "@type": "Answer", text: strip(x.a) },
    })),
  };
...
function injectMeta(html, pathname) {
  ...
  const ld = faqJsonLd(pathname);
  html = ld ? html.replace("</head>", ld + "</head>") : html;
  return html
